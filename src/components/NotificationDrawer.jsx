      ...
      headerAction={
        unread > 0 && (
          <button
            type="button"
            onClick={markAllRead}
            className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-semibold text-primary-fg hover:bg-primary-300/10"
          >
            <Check size={13} />
      ...
      footer={
        announcementHistoryUrl && (
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-lg py-1.5 text-sm font-semibold text-primary-fg hover:bg-primary-300/10"
          >
            <Megaphone size={15} />
      ...
      <ul>
        {items.map((n) => {
          ...
          return (
            <li
              key={n.id}
              className={`border-b border-border last:border-0 ${isUnread ? 'bg-primary-300/5' : ''}`}
            >
              {/* <button> (bukan onClick di <li>) supaya item bisa dioperasikan
                  keyboard dengan Enter/Space, bukan cuma mouse. */}
              <button
                type="button"
                onClick={() => handleItemClick(n, isUnread, payload.url)}
                className="flex w-full cursor-pointer items-start gap-3 px-5 py-3.5 text-left hover:bg-bg-page/50"
              >
                <span
                  className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
                    isUnread ? 'bg-primary-300/15 text-primary-fg' : 'bg-bg-page text-text-secondary'
                  }`}
                >
                  <Bell size={14} />
                </span>
