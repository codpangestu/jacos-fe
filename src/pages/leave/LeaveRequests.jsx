...
export default function LeaveRequests() {
  ...
  const user = getUser()
  const isMobileRole = user?.role === 'staff'
  ...
  return (
    <ResponsiveShell pageTitle={t('leave.title')} headerVariant="title" showSearch={false}>
      <form
        onSubmit={(e) => {
          e.preventDefault()
      ...
      >
        ...
        <div className={`grid gap-4 ${isMobileRole ? 'grid-cols-1' : 'sm:grid-cols-2'}`}>
          <FormField
            as="select"
            label={t('leave.type')}
            htmlFor="type"
          ...
          renderRow={(row) => (
            <div>
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-semibold text-text-primary">{t(`status.${row.type}`, row.type)}</p>
                <StatusBadge code={row.status} />
              </div>
              <p className="mt-1 text-xs text-text-secondary">
                {formatDate(row.start_date)} — {formatDate(row.end_date)}
              </p>
              {row.review_note && <p className="mt-1 text-xs text-text-secondary">{t('leave.reviewNote')}: {row.review_note}</p>}
            </div>
          )}
        />
      ) : (
